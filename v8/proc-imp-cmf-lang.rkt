...
(define-language proc-imp-cmf-lang-v8
  ...
  (structs
    [Def     ([name : Label] [params : (Listof Aloc)] [body : Entry])
             (define ,name (lambda ,params ,body))]
    [Program ([defs : (Listof Def)] [body : Entry])
             (module ,defs ... ,body)])
  ...
  (Effect
    [SetEffect   ([target : Aloc] [value : Value])
                 (set! ,target ,value)]
    [MSetEffect  ([base : Aloc] [index : Opand] [value : Triv])

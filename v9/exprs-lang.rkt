...
(define (prim-f? x)
  (and (symbol? x)
       (and (member x PRIMOPS) #t)))

(define-language exprs-lang-v8
  ...
  (unions
   [X     (Name PrimF)]
   [AtomTriv (X Fixnum Bool Empty AsciiCharLiteral)])

  (pairs
   [Binding ([name : X] [value : Value]) [,name ,value]])

  (structs
   [Def     ([name : X] [params : (Listof X)] [body : Value])
  ...
  (Triv
   ...
   [ErrorTriv ([code : UInt8])
              (error ,code)]
   [LambdaTriv ([params : (Listof X)] [body : Value])
               (lambda ,params ,body)]
   [#:ref AtomTriv])
